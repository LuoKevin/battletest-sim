...
describe('/pet/:petID_GET', function() {
  ...
  request(serverURL)
    .get('/pet/5').set({}).set('Cookie', ["lastVisitedDate=undefined"]).send()
    .expect(400)
    .end((err, res) => {
        endTime = Date.now();
        const resultData =  {
  ...
  request(serverURL)
    .get('/pet/9007199254740991').set({}).set('Cookie', ["lastVisitedDate=undefined"]).send()
    .expect(400)
    .end((err, res) => {
        endTime = Date.now();
        const resultData =  {
  ...
  request(serverURL)
    .get('/pet/1.7976931348623157e+308').set({}).set('Cookie', ["lastVisitedDate=undefined"]).send()
    .expect(400)
    .end((err, res) => {
        endTime = Date.now();
        const resultData =  {
  ...
  request(serverURL)
    .get('/pet/Infinity').set({}).set('Cookie', ["lastVisitedDate=undefined"]).send()
    .expect(400)
    .end((err, res) => {
        endTime = Date.now();
        const resultData =  {
  ...
  request(serverURL)
    ...
    it('{"header":{"token":"Random array of strings"}}', function(done){
        let endTime;
        const startTime = Date.now();
